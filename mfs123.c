...
int main()
{
 ...
 char *arg_ptr;
  ...
  {
   /*      execlp(token[0], token[0], token[1], token[2], token[3], token[4], token[5], token[6], token[7], token[8], token[9], token[10],(char *)NULL);
    execlp("/usr/local/bin", token[0], token[1], token[2], token[3], token[4], token[5], token[6], token[7], token[8], token[9], token[10],(char *)NULL);
	execlp("/usr/bin", token[0], token[1], token[2], token[3], token[4], token[5], token[6], token[7], token[8], token[9], token[10],(char *)NULL);
	execlp("/bin", token[0], token[1], token[2], token[3], token[4], token[5], token[6], token[7], token[8], token[9], token[10],(char *)NULL);       
*/
...
void shell_cd(char* dir){
	if(dir == NULL)         // Error checking, verifying user entered a directory
		fprintf(stderr,"Error: invalid option, \"cd\" requires a destination directory.\n");
	else
		if(chdir(dir) != 0)     // CHDIR() will change directory, if return != 0 then there was an error
			perror("cd");}
